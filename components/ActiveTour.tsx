...
const StatCard: React.FC<{ title: string; amount: number; color: string; textColor: string; icon: React.ReactNode }> = ({ title, amount, color, textColor, icon }) => (
    <div className={`p-5 rounded-xl shadow-lg flex items-center gap-5 border border-opacity-20 ${color} border-black`}>
        <div className="p-3 bg-white bg-opacity-30 rounded-full shadow-sm text-white">{icon}</div>
        <div>
            <p className={`text-sm font-bold uppercase tracking-wider ${textColor}`}>{title}</p>
            <p className="text-3xl font-extrabold text-white mt-1 drop-shadow-sm">
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center p-4 z-[60] backdrop-blur-sm animate-fade-in-up">
        <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm border-2 border-red-100">
            <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div>
                    <h3 className="text-xl font-extrabold text-gray-900">Delete Expense?</h3>
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center p-4 z-50 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg relative animate-fade-in-up border border-gray-200">
                ...
                <h3 className="text-2xl font-extrabold text-gray-900 mb-4">Confirm Tour Completion</h3>
        ...
        <div className="space-y-8 animate-fade-in-up">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-md border-l-8 border-indigo-600 flex justify-between items-center flex-wrap gap-4">
                <div>
                    ...
                    <div className="flex items-center gap-2 text-indigo-700 font-medium mt-2 bg-indigo-50 px-3 py-1 rounded-full w-fit">
                        <Calendar size={18} />
            ...
            <div className="grid md:grid-cols-3 gap-6">
                ...
                <StatCard title="Total Spent" amount={totalExpenses} color="bg-gradient-to-br from-orange-500 to-orange-600" textColor="text-orange-100" icon={<IndianRupee size={28} />} />
                 <div className={`p-5 rounded-xl shadow-lg flex items-center gap-5 border border-opacity-20 ${isOverspent ? 'bg-gradient-to-br from-red-500 to-red-600' : 'bg-gradient-to-br from-green-500 to-green-600'} border-black`}>
                    <div className="p-3 bg-white bg-opacity-30 rounded-full shadow-sm text-white"><IndianRupee size={28} /></div>
                    <div>
                        <p className="text-sm font-bold uppercase tracking-wider text-white opacity-90">Balance</p>
                        <p className="text-3xl font-extrabold text-white mt-1 drop-shadow-sm">
             ...
             <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                <div className="flex justify-between items-center p-6 bg-gray-50 border-b border-gray-200">
                    <h3 className="text-xl font-extrabold text-gray-800">Expense Log</h3>
                    <button onClick={handleAddClick} className="bg-indigo-600 text-white font-bold py-2.5 px-5 rounded-xl hover:bg-indigo-700 flex items-center gap-2 transition-all transform hover:scale-105 shadow-md">
                        <PlusCircle size={20} />
                        Add New
                    </button>
                </div>
                            ...
                            <div key={exp.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-5 hover:bg-gray-50 transition-colors duration-200 gap-4">
                                <div className="flex items-start gap-4">
                                    <div className="p-3 bg-indigo-50 rounded-xl text-indigo-600 mt-1">
                                        {React.cloneElement(CATEGORY_ICONS[exp.category], { size: 28 })}
                                    </div>
                                    <div>
                                        <p className="text-lg font-bold text-gray-900">{exp.category}</p>
                                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-sm font-medium text-gray-600">
                                            ...
                                            <span className="w-1 h-1 bg-gray-400 rounded-full"></span>
                                            <span className="text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded text-xs uppercase tracking-wide">{exp.paymentMode}</span>
                                        </div>
                                ...
                                <div className="flex items-center justify-between sm:justify-end gap-5 w-full sm:w-auto mt-2 sm:mt-0 pt-3 sm:pt-0 border-t sm:border-0 border-gray-100">
                                    ...
                                    <span className="text-xl font-bold text-white bg-indigo-600 px-4 py-2 rounded-lg shadow-sm font-mono tracking-wide">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pb-4">
                 {hasHistory && (
                    <button onClick={onToggleHistory} className={`w-full font-bold py-4 px-6 rounded-xl border flex items-center justify-center gap-3 transition-transform transform active:scale-95 shadow-sm ${isHistoryVisible ? 'bg-indigo-100 text-indigo-800 border-indigo-200' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}>
                        {isHistoryVisible ? <EyeOff size={22} /> : <History size={22} />}
                        {isHistoryVisible ? 'Hide Completed Tours' : 'View Past Tours History'}
                    </button>
                 )}
                <button onClick={() => setIsCompleteModalOpen(true)} className="w-full bg-green-600 text-white font-bold py-4 px-6 rounded-xl hover:bg-green-700 flex items-center justify-center gap-3 transition-transform transform active:scale-95 shadow-lg shadow-green-200">
                    <CheckCircle size={22} />
                    Complete & Close Tour
                </button>
            </div>
